...
class Slider<D> implements ChartBehavior<D> {
  ...
  ///
  /// This is used to track changes in the position of the slider caused by 
  ...
  Slider(
    ...
    _sliderEventListener =
         SliderEventListener<D>(onChange: onChangeCallback);
  }
  ...
  void _updateViewData() {
    ...
    _sliderHandle.setTarget(element);
  ...
  void _fireChangeEvent(_) {
    ...
    // since the last draw. We always set the initial drag state event when 
  ...
  bool _moveSliderToPoint(Point<double> point) {
    ...
    if (_chart != null) {
      ...
      if (_domainCenterPoint != null) {
        _domainCenterPoint =
             Point<int>(position.round(), _domainCenterPoint.y);
      } else {
...
class _AnimatedSlider<D> {
  ...
  ///
  /// This should be called in place of "setTarget" for points that represent
  /// data that has been removed from the series.
  ///
  /// Animates the width of the slider down to 0.
  void animateOut() {
    final Target = _currentSlider.clone();
    ...
    Target.buttonBounds =  Rectangle<int>(left.round(), top.round(),
        (right - left).round(), (bottom - top).round());
  ...
  void setTarget(_SliderElement<D> Target) {
    animatingOut = false;
    _currentSlider ??= Target.clone();
    _previousSlider = _currentSlider.clone();
    _targetSlider = Target;
  }
...
///
/// [initial] indicates that the slider was set to an initial position when 
