    ...
    annotations.forEach((AnnotationSegment annotation) {
      ...
      if (_annotationMap.containsKey(key)) {
        ...
        animatingAnnotation =  _AnimatedAnnotation<D>(key: key)
          ..setTarget( _AnnotationElement<D>()
      ...
      animatingAnnotation.setTarget(annotationElement);
    });
...
class _AnimatedAnnotation<D> {
  ...
  ///
  /// This should be called in place of "setTarget" for annotations have been
  /// removed from the list.
  /// TODO: Needed?
  void animateOut() {
    final Target = _currentAnnotation.clone();

    setTarget(Target);
    animatingOut = true;
  }

  void setTarget(_AnnotationElement<D> Target) {
    animatingOut = false;
    _currentAnnotation ??= Target.clone();
    _previousAnnotation = _currentAnnotation.clone();
    _targetAnnotation = Target;
  }
