...
class LineRenderer<D> extends BaseCartesianRenderer<D> {
  ...
  // previous draw cycles, but no longer have a corresponding datum in the 
  // data.
  ...
  void _mergeIntoSeriesMap(List<ImmutableSeries<D>> seriesList) {
    List<MapEntry<String, List<_AnimatedElements<D>>>> LineMap = [];
    ...
    seriesList.forEach((ImmutableSeries<D> series) {
      ...
      while (checkNext && _seriesLineMap.isNotEmpty) {
        ...
        if (!seriesList.any((s) => s.id == firstKey)) {
          LineMap.add(MapEntry(firstKey, _seriesLineMap.remove(firstKey)));
          checkNext = true;
        } else {
          checkNext = false;
        }
      ...
      if (!_seriesLineMap.containsKey(key)) {
        LineMap.add(MapEntry(key, []));
      } else {
        LineMap.add(MapEntry(key, _seriesLineMap.remove(key)));
      }
    });
    ...
    LineMap.addAll(_seriesLineMap.entries);
    _seriesLineMap.clear();

    _seriesLineMap.addEntries(LineMap);
  }
    ...
    seriesList.forEach((ImmutableSeries<D> series) {
      ...
      styleSegments.forEach((_LineRendererElement styleSegment) {
        ...
        if (animatingElements != null) {
          ...
          for (var index = 0; index < lineElementList.length; index++) {
            animatingLines.add( _AnimatedLine<D>(
              ...
              ..setTarget(lineElementList[index]));
          }
          ...
          if (config.includeArea) {
            ...
            for (var index = 0; index < areaElementList.length; index++) {
              animatingAreas.add( _AnimatedArea<D>(
                  ...
                  overlaySeries: series.overlaySeries)
                ..setTarget(areaElementList[index]));
            }
          }
          ...
          if (_hasMeasureBounds) {
            ...
            for (var index = 0; index < boundsElementList.length; index++) {
              animatingBounds.add( _AnimatedArea<D>(
                  ...
                  overlaySeries: series.overlaySeries)
                ..setTarget(boundsElementList[index]));
            }
          }
        ...
        for (var index = 0; index < lineElementList.length; index++) {
          ...
          animatingElements.lines[index].setTarget(lineElement);
        }
        ...
        if (config.includeArea) {
          for (var index = 0; index < areaElementList.length; index++) {
            ...
            animatingElements.areas[index].setTarget(areaElement);
          }
        }
        ...
        if (_hasMeasureBounds) {
          for (var index = 0; index < boundsElementList.length; index++) {
            ...
            animatingElements.bounds[index].setTarget(boundElement);
          }
        }
...
class _LineRendererElement<D> {
  ...
  _LineRendererElement<D> clone() {
    return  _LineRendererElement<D>()
      ...
      ..dashPattern =
          dashPattern != null ?  List<int>.from(dashPattern) : null
      ..domainExtent = domainExtent
  ...
  void updateAnimationPercent(_LineRendererElement previous,
    ...
    for (pointIndex = 0; pointIndex < target.points.length; pointIndex++) {
      ...
      // TODO: Can this be done in setTarget instead?
...
class _AnimatedLine<D> {
  ...
  void animateOut() {
    ...
    // animation is done.
    Target.strokeWidthPx = 0.0;

    setTarget(Target);
    animatingOut = true;
  }

  void setTarget(_LineRendererElement<D> Target) {
    animatingOut = false;
    _currentLine ??= Target.clone();
...
class _AnimatedArea<D> {
  ...
  void setTarget(_AreaRendererElement<D> Target) {
    animatingOut = false;
    _currentArea ??= Target.clone();
