  ...
  PointRenderer({String rendererId, PointRendererConfig config})
      : this.config = config ??  PointRendererConfig(),
        ...
        super(
            ...
            symbolRenderer:
                config?.symbolRenderer ??  CircleSymbolRenderer());
      ...
      for (var index = 0; index < series.data.length; index++) {
        ...
        if (animatingPoint == null) {
          ...
          animatingPoint =  AnimatedPoint<D>(
            ...
            ..setTarget( PointRendererElement<D>()
              ..color = details.color
        ...
        animatingPoint.setTarget(pointElement);
      }
    });
...
class AnimatedPoint<D> {
  ...
  /// (position of 0).
  void animateOut() {
    var Target = _currentPoint.clone();

    // Set the target measure value to the axis position.
    var targetPoint = Target.point;
    Target.point =  DatumPoint<D>.from(targetPoint,
        x: targetPoint.x,
        y: Target.measureAxisPosition.roundToDouble(),
        yLower: Target.measureAxisPosition.roundToDouble(),
        yUpper: Target.measureAxisPosition.roundToDouble());
  ...
  void setTarget(PointRendererElement<D> Target) {
    animatingOut = false;
    _currentPoint ??= Target.clone();
